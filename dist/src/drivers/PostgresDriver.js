...
class PostgresDriver extends AbstractDriver_1.AbstractDriver {
    constructor() {
        ...
        this.GetAllTablesQuery = (schema) => __awaiter(this, void 0, void 0, function* () {
            let response = (yield this.Connection.query(`SELECT table_schema as "TABLE_SCHEMA",table_name as "TABLE_NAME" FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='BASE TABLE' AND table_schema in (${schema}) `)).rows;
            return response;
        });
    }
            ...
            entities.forEach(ent => {
                response
                    ...
                    colInfo.default = colInfo.is_generated
                        ? null
                        : resp.column_default;
                    var columnTypes = this.MatchColumnTypes(resp.data_type, resp.udt_name);
                    ...
                    if (colInfo.is_array) {
                        colInfo.ts_type = colInfo.ts_type
                            .split("|")
    ...
    MatchColumnTypes(data_type, udt_name) {
        let ret = { ts_type: null, sql_type: null, is_array: false };
